...
- Canonical entry point consolidation: Map modern agents adopting AGENTS.md (such as Claude Code and Codex) directly to AGENTS.md in IDE_ENTRY_POINTS to eliminate redundant tool-specific files and maintain a single canonical root contract.
- Template skills indexing: Provide a self-contained skills/index.md in each framework template (and shared) to catalog available skills and triggers; the merge engine naturally overlays the template catalog over the base shared catalog without CLI code churn.
